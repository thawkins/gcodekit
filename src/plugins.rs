...
impl PluginManager {
    ...
    pub fn load_plugin(&mut self, mut plugin: Box<dyn Plugin>) -> Result<(), String> {
        ...
        self.plugin_states.insert(plugin_id.clone(), PluginState::Loading);
        ...
        match plugin.initialize(&self.context) {
            ...
            Err(e) => {
                self.plugin_states.insert(plugin_id, PluginState::Error(e.clone()));
    ...
    pub fn unload_plugin(&mut self, plugin_id: &str) -> Result<(), String> {
        if let Some(mut plugin) = self.plugins.remove(plugin_id) {
            match plugin.shutdown() {
                Ok(()) => {
                    self.plugin_states.insert(plugin_id.to_string(), PluginState::Unloaded);
                    Ok(())
                }
                Err(e) => {
                    self.plugin_states.insert(plugin_id.to_string(), PluginState::Error(e.clone()));
    ...
    pub fn get_plugin_state(&self, plugin_id: &str) -> PluginState {
        self.plugin_states.get(plugin_id).cloned().unwrap_or(PluginState::Unloaded)
    }
...
impl Plugin for ExamplePlugin {
    ...
    fn on_gcode_loaded(&mut self, gcode: &str) {
        println!("Example plugin: G-code loaded, {} lines", gcode.lines().count());
    }

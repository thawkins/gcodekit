...
impl WebPendant {
    pub fn new() -> (Self, CommandReceiver) {
        ...
        let status_receivers = Arc::new(Mutex::new(HashMap::<String, mpsc::UnboundedSender<WebPendantMessage>>::new()));
    ...
    pub fn send_status_update(&self, connected: bool, position: Option<(f32, f32, f32)>, status: String) {
    ...
    pub async fn start_server(&self, port: u16) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let ws_route = warp::path("ws")
            ...
            .map(move |ws: warp::ws::Ws| {
                ...
                ws.on_upgrade(move |socket| Self::handle_websocket(socket, status_receivers, command_sender))
            });

        // Main HTML page
        let index = warp::path::end()
            .map(|| {
                warp::reply::html(Self::get_html_page())
            });
